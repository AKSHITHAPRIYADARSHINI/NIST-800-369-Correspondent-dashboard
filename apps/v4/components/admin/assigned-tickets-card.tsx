...
import type { AssignedTicketItem } from "@/lib/mock-data/admin-tickets"
import { Button } from "@/registry/new-york-v4/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/registry/new-york-v4/ui/card"
import { Checkbox } from "@/registry/new-york-v4/ui/checkbox"

export function AssignedTicketsCard({ tasks }: { tasks: AssignedTicketItem[] }) {
        ...
        {state.map((task) => (
          <div key={task.id} className="rounded-md border border-border/70 p-2">
            <div className="flex items-start gap-2">
              <Checkbox checked={task.done} onCheckedChange={(checked) => setState((prev) => prev.map((row) => row.id === task.id ? { ...row, done: Boolean(checked) } : row))} />
